...
pub async fn execute_repository_pipeline(
    ...
    if !crawled_items.is_empty() {
        ...
        for node in &config.nodes {
            ...
            match source_type {
                "url" | "api" => {
                    let mut url = source_value.to_string();
                    let profile = extract_client_profile(&node.data);

                    // Dynamic storeId checkout via reqwest
                    if url.contains("oreka.vn") && url.contains("/store/") {
                        log_manager.info(
                            project_id,
                            "fetching",
                            &format!(
                                "[node={}] Fetching store page via reqwest to resolve storeId: {}",
                                node_label, url
                            ),
                        );
                        ...
                        if let Ok(c) = client {
                            if let Ok(resp) = c.get(&url).send().await {
                                if let Ok(html_body) = resp.text().await {
                                    ...
                                    // Parse __NEXT_DATA__
                                    if let Some(caps) = regex::Regex::new(r#"<script\s+id=["']__NEXT_DATA__["']\s+type=["']application/json["'][^>]*>(.*?)</script>"#)
                                        .ok()
                                        .and_then(|re| re.captures(&html_body))
                                    {
                                        if let Some(json_str) = caps.get(1).map(|m| m.as_str()) {
                                            if let Ok(next_json) = serde_json::from_str::<serde_json::Value>(json_str) {
                                                store_id = next_json.pointer("/props/pageProps/store/id")
                                                    .and_then(|v| v.as_str())
                                                    .map(|s| s.to_string());
                                                if store_id.is_none() {
                                                    // search recursively
                                                    fn find_id(val: &serde_json::Value) -> Option<String> {
                                                        if let Some(obj) = val.as_object() {
                                                            if let Some(id) = obj.get("store").and_then(|s| s.get("id")).and_then(|v| v.as_str()) {
                                                                return Some(id.to_string());
                                                            }
                                                            if let Some(id) = obj.get("storeId").and_then(|v| v.as_str()) {
                                                                return Some(id.to_string());
                                                            }
                                                            for v in obj.values() {
                                    ...
                                    if let Some(id) = store_id {
                                        let parsed_base = if let Some(caps) =
                                                ...
                                                .and_then(|re| re.captures(&url))
                                        {
                                            ...
                                            "https://www.oreka.vn".to_string()
                                        };
                                        url = format!(
                                            "{}/mua-ban?storeId={}&sort=createdAt&order=desc",
                                            parsed_base.trim_end_matches('/'),
                                            id.trim()
                                        );
                                        log_manager.info(
                                            project_id,
                                            "fetching",
                                            &format!("[node={}] Resolved storeId: {}. Rewriting crawl URL to: {}", node_label, id, url),
                                        );
                                    } else {
                                        log_manager.warn(
                                            project_id,
                                            "fetching",
                                            &format!("[node={}] Could not find storeId in store page HTML. Using original URL.", node_label),
                                        );
                                    }
                                }
                            }
