...
class TauriPlatform implements PlatformAdapter {
  ...
  private async _ensureMasterSchema(db: Database): Promise<void> {
    const sql = [
      ...
      `CREATE TABLE IF NOT EXISTS project_runtime (project_id TEXT PRIMARY KEY, runner_status TEXT NOT NULL DEFAULT 'stopped', runner_pid INTEGER, runner_type TEXT DEFAULT 'service', service_control TEXT NOT NULL DEFAULT 'run', edit_pid INTEGER, cycle_count INTEGER NOT NULL DEFAULT 0, last_run_at TEXT, last_error TEXT, updated_at TEXT NOT NULL DEFAULT (datetime('now')))`,
    ];
    for (const s of sql) {
      await db.execute(s);
    }
...
class BrowserPlatform implements PlatformAdapter {
  ...
  private _ensureMasterSchema(db: InMemoryDatabase): void {
    ...
    db.execute(`CREATE TABLE IF NOT EXISTS project_runtime (project_id TEXT PRIMARY KEY, runner_status TEXT NOT NULL DEFAULT 'stopped', runner_pid INTEGER, runner_type TEXT DEFAULT 'service', service_control TEXT NOT NULL DEFAULT 'run', edit_pid INTEGER, cycle_count INTEGER NOT NULL DEFAULT 0, last_run_at TEXT, last_error TEXT, updated_at TEXT NOT NULL DEFAULT (datetime('now')))`);
